...
char most_common (string str){
  unordered_map<char, int> freqs = {
    ...
    {'9',0},
    {'T',0},
    {'J',-4}, //only return Joker as most common if there are 5 Jokers
    {'Q',0},
    {'K',0},
    {'A',0},
  };
  ...
  int max = 0;
  char most_common = 'J';
  for (auto element : freqs) {
    if (element.second>max) {
      max = element.second;
      most_common = element.first;
    } 
  }
...
int get_rank(string hand) {
  ...
  char joker = most_common(hand);

  if (joker != 'J') {
    std::replace(hand.begin(), hand.end(), 'J', joker);  
  }else {
    std::replace(hand.begin(), hand.end(), 'J', 'A');  
  }
